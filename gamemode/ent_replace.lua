...
if (SERVER) then
   hook.Add("TTTPrepareRound", "RemoveAllAttachments", function()
      ...
      for k,v in pairs(player.GetAll()) do
         for i,j in pairs(scripted_ents.GetList()) do
            if(j.t.AttachmentName != nil) then
               v:SetNWInt("Has" ..j.t.AttachmentName, 0);
            end
         end
      end
         ...
         for _, ent in pairs(ents.GetAll()) do
            if (ent.RegisteredAttachments == nil || table.Count(ent.RegisteredAttachments) == 0) then continue; end;
            local attList = {};

            for a,b in pairs(ent.RegisteredAttachments) do
               for c,d in pairs(b) do
                  local attData = GetAttachment(c);
                  if (!attData.Weight) then
                     attData.Weight = 1;
                  end
                  table.insert(attList, {c, attData.Weight});
               end
            end

            local spawnedAtt = nil;
            local foundAtt = false;
            local testAttCount = 0;

            local randTblKey = math.random(1, table.Count(attList));

            while(!foundAtt && attList[randTblKey] != nil && attList[randTblKey][2] != nil && testAttCount < table.Count(attList)) do
               if (attList[randTblKey] && attList[randTblKey][2] && attList[randTblKey][2] >= (math.random(1, 100) / 100 ) ) then
                  spawnedAtt = GetAttachment(attList[randTblKey][1]);
                  foundAtt = true;
               else
                  randTblKey = math.random(1, table.Count(attList));
                  testAttCount = testAttCount + 1;
               end
            end
            if (testAttCount >= table.Count(attList)) then
               --print("Nothing spawned at " ..tostring(ent));
               continue;
            end

            if (spawnedAtt != nil) then
               local att = ents.Create(spawnedAtt.ClassName)
               local pos = ent:GetPos();
               pos.z = pos.z + 3
               att:SetPos(pos)
               att:SetAngles(VectorRand():Angle())
               att:Spawn()
               --print("Spawned " ..spawnedAtt.ClassName)
            else
               --print(tostring(ent).. " has no attachments")
            end
         end
      end)
   end)
   ...
   function GetAttachment(name)
      for a,b in pairs(scripted_ents.GetList()) do
         if (b.t.AttachmentName && name == b.t.AttachmentName) then
            return b.t;
         end
      end
   end
...
function ents.TTT.GetSpawnableAmmo()
   if not SpawnableAmmoClasses then
      ...
      for k,v in pairs(scripted_ents.GetList()) do
         if (v && v.AutoSpawnable || (v.t && v.t.AutoSpawnable) && v.t.AttachmentName == nil) then
            table.insert(tbl, k)
         end
      end
